...
<body>
  ...
  <div class="">
    <div class="p-3 mb-2 bg-primary text-white" style="margin-top: 30px;"><?= $login->NgonNgu == 1 ? "Quản lý Ưu đãi" : "Voucher Manager" ?></div>
    <a href="?act=createvoucher"><button class="btn btn-success mb-3" data-toggle="modal" data-target="#addLoaiModal">Thêm Voucher</button></a>
      ...
      <tbody>
        <?php foreach ($Voucher as $type) { ?>
          <tr>
            ...
            <th><?= $type->url?></th>
            <th>
              <a href="?act=edituudai&id=<?= $type->id ?>"><button class="btn btn-primary">Sửa</button></a>
              <a href="?act=delltype&id=<?= $type->id ?>"><button class="btn btn-danger" onclick="return confirm('Bạn có chắc muốn xóa không')">Xóa</button></a>
            </th>
          </tr>
        <?php } ?>
      </tbody>
